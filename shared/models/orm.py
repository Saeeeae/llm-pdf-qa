...
class LLMConfig(Base):
    ...
    vllm_url = Column(String(255), nullable=False, default="http://vllm-server:8000/v1")
...
class EventLog(Base):
    """General-purpose extensible event log for all modules.

    Complements specialized tables (pipeline_logs, query_logs, etc.)
    with a unified, queryable event stream. The `details` JSONB field
    makes it easy to add new fields without schema changes.
    """
